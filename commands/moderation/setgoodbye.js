...
module.exports = {
    name: 'setgoodbye',
    description: 'Vous permet d\'activer et configurer le système d\'aurevoir.',
    usage: '<salon/off>',
    guildOnly: true,
    cooldown: 5,
    args: true,
            ...
            if (args[0].toLowerCase() === 'off' || args[0].toLowerCase() === 'remove') {
                if (data.onoff === false) {
                    return message.channel.send(new Discord.MessageEmbed()
                        .setDescription("<:icone_indisponible:824290381222117396> | Le système d'aurevoir est déjà désactivé sur ce serveur.")
                        .setColor('d10000')
                    );
                } else {
                    data.onoff = false;
                    ...
                    message.channel.send(new Discord.MessageEmbed()
                        .setDescription(`<:icone_verified:824290381435895818> | ${message.author}, le Système d\'Aurevoir est désormais désactivé sur ce serveur.`)
                        .setColor('14b321')
                    );
                }
            } else {
                ...
                const embed = new Discord.MessageEmbed()
                    .setTitle("Système d\'Aurevoir Config \n\n")
                    .setDescription("\n\n")
                    .setColor(roleColor)
                    .setDescription(`Le salon d'aurevoir sera désormais celui-ci : <#${salon.id}>, êtes vous d'accord ? Appuyez sur les réactions pour valider votre choix.`)
                    .addFields(
                    ...
                    .setFooter('Tools Système d\'Aurevoir', client.user.displayAvatarURL({ format: "png", dynamic: true }))
                    .setTimestamp()
                ...
                collector.on('collect', async (reaction, user) => {
                    if (reaction.emoji.id === '826378817680703508') {
                        ...
                        message.channel.send(new Discord.MessageEmbed()
                            .setDescription(`<:icone_verified:824290381435895818> | ${user}, le Système d\'Aurevoir est désormais activé sur ce serveur, dans le salon <#${salon.id}>.`)
                            .setColor('14b321')
                        );
